...
Black.text((110, 250), TemHigh[0] + u'°C~' +
           TemLow[0] + u'°C', font=font30, fill="BLUE")
...
Black.text((0+(180-len(WinSpeed[0])*10), 370),
           WinSpeed[0], font=font30, fill="BLUE")
...
for i in range(1, 6):
    ...
    Black.text((380+(i-1)*180, 250),
               TemHigh[i] + u'°C~' + TemLow[i] + u'°C', font=font30, fill="BLUE")
    if (len(Wea[i]) < 6):
        Black.text(
            (360+(i-1)*180+(90-len(Wea[i])*15), 290), Wea[i], font=font30, fill="BLUE")
    else:
        Black.text(
            (360+(i-1)*180+(90-len(Wea[i])*13), 290), Wea[i], font=font25, fill="BLUE")
    if (len(Win[i]) < 6):
        Black.text(
            (360+(i-1)*180+(90-len(Win[i])*15), 330), Win[i], font=font30, fill="BLUE")
    else:
        Black.text(
            (360+(i-1)*180+(90-len(Win[i])*13), 330), Win[i], font=font25, fill="BLUE")

    Black.text((360+(90-len(WinSpeed[i])*10)+(i-1) *
                180, 370), WinSpeed[i], font=font30, fill="BLUE")
    Black.rectangle([(430+(i-1)*180, 409), (470+(i-1)*180, 443)], fill="BLUE")
    Black.text((435+(i-1)*180, 410),
               Level[9+(i-1)*5], font=font30, fill="WHITE")
###############################################################################################
...
for i in range(0, 7):
    ...
    arc_dax = 4
    Other.ellipse([120+150*i-arc_dax, y1-arc_dax, 120 +
                   150*i+arc_dax, y1+arc_dax], fill="BLUE")
    Other.ellipse([120+150*(i+1)-arc_dax, y2-arc_dax, 120 +
                   150*(i+1)+arc_dax, y2+arc_dax], fill="BLUE")
    Other.line([(120+150*i, y1), (120+150*(i+1), y2)], fill="BLUE", width=3)

    Other.text((120+150*i, y1-30),
               TodayTem[k+i]+u'°C', font=font25, fill="BLUE")
    if(i == 6):
        Other.text((120+150*(i+1), y2-30),
                   TodayTem[k+i+1]+u'°C', font=font25, fill="BLUE")
Other.text((20, 850-60),  str(Tem_min+int((Tem_max - Tem_min)*0/4)),
           font=font30, fill="BLUE")
Other.text((20, 850-120), str(Tem_min+int((Tem_max - Tem_min)*1/4)),
           font=font30, fill="BLUE")
Other.text((20, 850-180), str(Tem_min+int((Tem_max - Tem_min)*2/4)),
           font=font30, fill="BLUE")
Other.text((20, 850-240), str(Tem_min+int((Tem_max - Tem_min)*3/4)),
           font=font30, fill="BLUE")
Other.text((20, 850-300), str(Tem_min+int((Tem_max - Tem_min)*4/4)),
           font=font30, fill="BLUE")
