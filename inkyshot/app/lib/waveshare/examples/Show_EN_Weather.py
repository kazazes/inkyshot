...
if not os.path.exists(picdir):
    picdir = os.path.abspath('/usr/app/img')
...
Painting.text((117-len(Tem)*50, H_Proportion/3),
              Tem, font=font110, fill="BLUE")
Black.text((117-len(Tem)*50+120, H_Proportion/3),
           u'°F', font=font40, fill="BLUE")

Black.text((W_Proportion, H_Proportion/5*3),
           OtherData[0], font=font30, fill="BLUE")
Black.text((W_Proportion, H_Proportion/5*4),
           OtherData[1], font=font30, fill="BLUE")

Black.text((int(W_Proportion*2.5), H_Proportion/5*3),
           OtherData[2], font=font30, fill="BLUE")
Black.text((int(W_Proportion*2.5), H_Proportion/5*4),
           OtherData[3], font=font30, fill="BLUE")

Black.text((int(W_Proportion*4), H_Proportion/5*3),
           OtherData[4], font=font30, fill="BLUE")
Black.text((int(W_Proportion*4), H_Proportion/5*4),
           OtherData[5], font=font30, fill="BLUE")

for i in range(0, Inage_WIDTH, 10):
    Black.ellipse([i-2, H_Proportion-20-2, i+2,
                   H_Proportion-20+2], fill="BLUE")
#############################################################
for i in range(0, 6):
    if (i != 0):
    ...
    PNG = Image.open(picdir + "/weather/" + str(i) + ".png")
...
for i in range(0, 24, 1):  # w == 50
    ...
    if(i != 23):
        y2 = Inage_HEIGHT-30-30 - \
            int((int(TemHourly[i+1])-tem_min)*tem_Proportion)
        Painting.line([(50+50*i, y1), (50+50*(i+1), y2)], fill="BLUE", width=3)
    else:
        Painting.text((50+50*i, y1-30),
                      TemHourly[i]+u'°F', font=font20, fill="BLUE")
        Black.ellipse([50+50*i-arc_dax, y1-arc_dax, 50+50 *
                       i+arc_dax, y1+arc_dax], fill="BLUE")
        Black.text((50+50*i, Inage_HEIGHT-27),
                   TimeHourly[i], font=font25, fill="BLUE")
...
Black.text((10, Inage_HEIGHT-30-30),  str(tem_min +
                                          int((tem_max - tem_min)*0/4)), font=font30, fill="BLUE")
Black.text((10, Inage_HEIGHT-30-90),  str(tem_min +
                                          int((tem_max - tem_min)*1/4)), font=font30, fill="BLUE")
Black.text((10, Inage_HEIGHT-30-150), str(tem_min +
                                          int((tem_max - tem_min)*2/4)), font=font30, fill="BLUE")
Black.text((10, Inage_HEIGHT-30-210), str(tem_min +
                                          int((tem_max - tem_min)*3/4)), font=font30, fill="BLUE")
Black.text((10, Inage_HEIGHT-30-270), str(tem_min +
                                          int((tem_max - tem_min)*4/4)), font=font30, fill="BLUE")
#############################################################
Display()
Display_END()
